...
class NonCustodialBarterContract(sp.Contract):
    """This contract implements a simple non-custodial barter contract where
    users can trade FA2 tokens for other FA2 tokens.
    ...
    def __init__(self, metadata):
        ...
        self.init_type(sp.TRecord(
            ...
            trades=sp.TBigMap(sp.TNat, NonCustodialBarterContract.TRADE_TYPE),
            counter=sp.TNat))
    ...
    def propose_trade(self, trade_proposal):
        ...
        sp.set_type(trade_proposal, sp.TRecord(
            tokens=NonCustodialBarterContract.TOKEN_LIST_TYPE,
            for_tokens=NonCustodialBarterContract.TOKEN_LIST_TYPE,
        ...
        with sp.for_("token", trade_proposal.tokens) as token:
            sp.verify(token.amount >= 0,
                      message="At least one token edition needs to be traded")

        with sp.for_("token", trade_proposal.for_tokens) as token:
            sp.verify(token.amount >= 0,
                      message="At least one token edition needs to be traded")
    ...
    def accept_trade(self, trade_id):
        ...
        with sp.if_(trade.user2.is_some()):
    ...
    def transfer_tokens(self, from_, to_, tokens):
        ...
        """
        with sp.for_("token", tokens) as token:
            self.fa2_transfer(
                fa2=token.fa2,
                from_=from_,
                to_=to_,
...
sp.add_compilation_target("nonCustodialBarter", NonCustodialBarterContract(
